...
function agregarNumeros(numeros) {
  ...
  // Tu código:
  return numeros.reduce(function(accumulator, currentValue) {
...
function promedioResultadosTest(resultadosTest) {
  ...
  // Tu código:
  return resultadosTest.reduce(function(accumulator, currentValue) {
    return accumulator + currentValue;
...
function todosIguales(arreglo) {
  ...
  //Escribe tu código aquí  
  let sonIguales = arreglo.filter(x => x === arreglo[0]);
  if(sonIguales.length === arreglo.length) {
    return true;
  }
  return false;  
} 
...
function breakStatement(numero) {
  ...
  // Tu código:
  let i = 0, array = [], interrupted = false;
  while(i < 10) {
    if(numero === 10){
      interrupted = true;
      break;
